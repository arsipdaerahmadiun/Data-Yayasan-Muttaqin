...
/* Text Colors - make sure ALL text inside dark mode goes white/gray */
.dark, .dark * {
  /* color: inherit;  <- removed this, might be too aggressive */
}

.dark .text-slate-900, .dark .text-slate-800, .dark .text-slate-950 {
...
/* Borders & Dividers */
.dark .border-slate-200, .dark .border-slate-100, .dark [class*="border-slate-200/"] {
  border-color: #2b3036 !important;
}
